...
void init_simulation(SimulationEngine* engine, Memory* memory, MutexController* mutex_ctrl, 
    ...
    for (int i = 0; i < process_count; i++) {
        processes[i]->queue_arrival_cycle = engine->clock_cycle;
...
int execute_cycle(SimulationEngine* engine, Process* current_proc) {
    LOG("Cycle %d: Starting with PID %d",
        engine->clock_cycle,
    ...
    while (instructions_executed < max_quantum &&
        ...
        LOG("Executing instruction %d/%d for PID %d: %s",
            instructions_executed+1, max_quantum,
        ...
        result = execute_instruction(
                    ...
                    &engine->ready_queues[
                        engine->algorithm == 2
                          ...
                          : 0]);
        LOG("Instruction result=%d, quantum_used=%d",
    ...
    if (result == -1) {
        LOG("PID %d blocked", current_proc->pcb.process_id);

    } 
    else if (result == 0) {
        LOG("PID %d terminated", current_proc->pcb.process_id);
          ...
          || (engine->algorithm == ALG_MLFQ && instructions_executed >= max_quantum)) 
    {
        ...
        int qidx = (engine->algorithm == ALG_MLFQ)
                 ? current_proc->pcb.priority - 1
                 : 0;   // RR always queue 0
        current_proc->queue_arrival_cycle = engine->clock_cycle;
